...
<main>
    <div class="one_product container">
        <div>
            <a class="btn_back" href="<?php echo e($backUrl); ?>"><img src="<?php echo e(asset('assets/images/oneProduct/back.svg')); ?>" alt="Назад"></a>
        </div>

        <div class="product_image product_image_magnifier" data-magnifier>
            <span class="product_image_frame">
                <img src="<?php echo e(asset($picture->img)); ?>" alt="<?php echo e($picture->name); ?>" data-magnifier-image>
            </span>
        ...
        <div class="product_info">
            ...
            <div class="description_accordion">
                ...
                <?php if($descriptionLength > 70): ?>
                    <?php
                        $temp = mb_substr($description, 0, 70);
                        $lastSpace = mb_strrpos($temp, ' ');
                        $short = $lastSpace !== false ? mb_substr($description, 0, $lastSpace) : $temp;
                        $remaining = $lastSpace !== false ? mb_substr($description, $lastSpace + 1) : mb_substr($description, 70);
                    ?>
            ...
            <?php if($picture->listing_type !== 'auction'): ?>
                <div class="product_buttons">
                    <?php if(session()->has('user_id') && session('user_id') == $picture->user_id): ?>
                        ...
                        <?php if (! ($is_sold)): ?>
                            <a href="<?php echo e(url('/add?auction_from=' . $picture->id)); ?>" class="auction_product_btn">
                                Добавить в аукцион
                            </a>
                        <?php endif; ?>
                    <?php else: ?>
                            ...
                            <div class="fav <?php echo e($is_in_favorites ? 'active' : ''); ?>" data-picture-id="<?php echo e($picture->id); ?>" data-is-favorite="<?php echo e($is_in_favorites ? 'true' : 'false'); ?>" style="cursor: pointer;">
                                ...
                                <div class="fav_count">
                                    ...
                                    <span class="likes-text"><?php
                                        $lastDigit = $likes_count % 10;
                                        $lastTwoDigits = $likes_count % 100;
                        ...
                        <?php else: ?>
                            <?php if(session()->has('user_id')): ?>
                                <div
                                    class="cart <?php echo e($isUserBlocked ?? false ? 'cart-disabled' : ''); ?>"
                                    id="addToCartBtn"
                                    data-picture-id="<?php echo e($picture->id); ?>"
                                    <?php if($isUserBlocked ?? false): ?> data-blocked-message="<?php echo e($cartRestrictionMessage); ?>" title="<?php echo e($cartRestrictionMessage); ?>" <?php endif; ?>
                                >
    ...
    <div class="delete_modal">
        ...
        <div class="delete_buttons">
            <button class="delete_btn delete_btn_back" onclick="closeDeleteModal()">
                <img src="<?php echo e(asset('assets/images/add/Left.svg')); ?>" alt="Назад"> Назад
            </button>
...
<style>
.edit_product_btn, .delete_product_btn, .auction_product_btn { display: flex; align-items: center; gap: 10px; padding: 15px 20px; border-radius: 20px; font-family: 'InterTight', sans-serif; font-size: 16px; font-weight: 400; cursor: pointer; transition: all 0.3s; border: none; text-decoration: none; }
...
.auction_product_btn { background: #FBFF83; color: #0D0D0D; border: 1px solid #FBFF83; }
.auction_product_btn:hover { background: #0D0D0D; color: #FBFF83; }
...
function getLikesWord(count) {
    ...
    if (lastDigit === 1) return 'лайк';
    if (lastDigit >= 2 && lastDigit <= 4) return 'лайка';
    return 'лайков';
}

document.addEventListener('DOMContentLoaded', function() {
    const accordionToggle = document.querySelector('.description_toggle');
    const hiddenDescription = document.querySelector('.description_hidden');
    if (accordionToggle && hiddenDescription) {
        accordionToggle.addEventListener('click', function(event) {
            event.preventDefault();
            event.stopImmediatePropagation();
            hiddenDescription.classList.toggle('active');
            accordionToggle.textContent = hiddenDescription.classList.contains('active') ? 'Скрыть' : 'Читать далее';
        }, true);
    }
        ...
        favButton.addEventListener('click', async function() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (likesNumber && likesText) {
                        likesNumber.textContent = result.likes_count;
                        likesText.textContent = getLikesWord(result.likes_count);
                    }
                }
            } catch (error) {
            } finally {
...
function showDeleteModal(id, name) {
    pictureToDelete = id;
    document.getElementById('deleteTitle').textContent = `Удалить картину "${name}"?`;
